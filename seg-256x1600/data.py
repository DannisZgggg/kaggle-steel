...
def TTA(image,cls_model,seg_model):
    #get average of multi version of test image augmentation
    #image: img:[n,3,256,256] CPU
    #model: segmentation model
    #output: pred_result:[n,4,256,256] CPU
    ...
    for tran in trans_zoo:
        ...
        cls_aug = cls_model(img_aug)
        cls_aug = F.sigmoid(cls_aug)
        cls_total += cls_aug.cpu()

    seg_result = seg_total/len(trans_zoo) #[n,4,256,256]
    cls_result = cls_total/len(trans_zoo) #[n,4]
...
def get_transforms(phase, mean, std):
    ...
    if phase == "train":
        list_transforms.extend([
                ...
                #GaussNoise(p=0.5,var_limit=(1., 30.)),
...
def compress(image,mask):
    ...
    if select_number < 0.6:
        ...
        dip1 = 200*random.random()
        dip2 = 200*random.random()
        pts1 = np.float32([[50, 50], [1500, 50], [50, 200]])
        pts2 = np.float32([[50 + dip1, 50], [1500 - dip2, 50], [50 + dip1, 200]])
