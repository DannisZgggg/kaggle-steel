...
def aug_patch_smooth(img,mask):
    ...
    img_tran = res['image']  # [256,1600]
    mask_tran = res['mask']  # [256,1600,4]
...
class TrainvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if aug_patch and (seed < patch_prob) and self.phase == 'train':
            ...
            img = np.concatenate((img, img, img), axis=-1) #[256,1600,3]

        img,mask = compress(img,mask)
        augmented = self.transforms(image=img, mask=mask)  # [256,1600,3],[256,1600,4]
        img = augmented['image'].squeeze().astype(np.float32)  # [256,1600,3]
        mask = augmented['mask'].squeeze().astype(np.float32)  # [256,1600,4]
        ...
        #Normalize
        list_trfms = Compose([Normalize(mean=self.mean, std=self.std, p=1)])
        img = list_trfms(image = img)['image']
        #[256,1600,3]->[3,256,1600]
        ...
        # img[3,256,1600]
        mask = mask.transpose((2,0,1))# [4,256,1600]
        label = (mask.sum(-1).sum(-1) > 0).astype(np.float32)
...
#     pts1 = np.float32([[50, 50], [1500, 50], [50, 200]])
#     if select_number < 0.4:
#         return image,mask
#     if 0.4 <= select_number < 0.7:
#         dip = 200*random.random()
#         pts2 = np.float32([[50 + dip, 50], [1500 - dip, 50], [50 + dip, 200]])
#     else:
#         dip = 25*random.random()
#         pts2 = np.float32([[50, 50 + dip], [1500, 50 + dip], [50, 200 - dip]])
#     M = cv2.getAffineTransform(pts1, pts2)
#     image = cv2.warpAffine(image, M, (1600, 256))
#     mask = cv2.warpAffine(mask, M, (1600, 256))
#     return image,mask
...
def compress(image,mask):
    ...
    if select_number < 0.6:
        ...
        dip1 = 100*random.random()
    else:
        dip2 = 100*random.random()
    pts1 = np.float32([[15, 15], [1500, 15], [15, 1500]])
    pts2 = np.float32([[15 + dip1, 15], [1500 - dip2, 15], [15 + dip1, 1500]])
    M = cv2.getAffineTransform(pts1, pts2)
    image = cv2.warpAffine(image, M, (1600, 256))
    mask = cv2.warpAffine(mask, M, (1600, 256))
